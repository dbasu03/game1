...
const App = () => {
  ...
  const [score, setScore] = useState(0);
  const [isGameOver, setIsGameOver] = useState(false);
  const [isMobile, setIsMobile] = useState(false); // Track whether it is mobile or not
  ...
  useEffect(() => {
    ...
    const checkDevice = () => {
      if (window.innerWidth <= 768) {
        setIsMobile(true); // Assume mobile for widths <= 768px
      } else {
        setIsMobile(false); // Assume desktop for widths > 768px
      }
    };
    ...
    // Initially check device
    checkDevice();
    
    // Recheck device on window resize
    window.addEventListener("resize", checkDevice);

    // Attach event listeners
    if (isMobile) {
      window.addEventListener("touchstart", handleTouchStart); // Mobile touch event
    } else {
      window.addEventListener("keydown", handleKeyDown); // Desktop keyboard event
    }
  ...
  return (
    <div
      style={{
        width: isMobile ? "100%" : "400px", // Full screen for mobile, fixed width for desktop
        height: isMobile ? "100vh" : "500px", // Full screen height for mobile, fixed height for desktop
        margin: "0 auto",
        position: "relative",
        background: "linear-gradient(to bottom, yellow, orange, red)", // Sunset gradient
        overflow: "hidden",
        border: isMobile ? "none" : "2px solid black", // Remove border for mobile
        touchAction: "none", // Disable default touch gestures for smoother gameplay on mobile
      }}
    >
      <Bird birdY={birdY} />
      <Pipes pipes={pipes} />
